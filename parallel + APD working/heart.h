...
private:
	...
	std::vector<std::vector<int>> total_refractory;
...
public:
	...
	void operate()//one time step
	{
		...
		for (int i = 0; i<L; i++)
		{
			for (int j = 0; j<L; j++)
			{
				...
				if (state[i][j] == 1)
				{
					total_refractory[i][j] += last_excited[i][j]; //count how long the cell was resting when it is next excited
				}
	...
	void set_total_refractory(int i, int j)
	{
		total_refractory[i][j] += last_excited[i][j];
	}
...
/*	double get_avg_refractory()
	...
	*/
	double get_avg_refractory(int i,int j, int time_steps)
	{
