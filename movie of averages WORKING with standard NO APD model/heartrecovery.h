...
class heart
{
	...
	public:
		...
		heart(const int _L,  int _T, int _tau ,double _nu, double _epsilom, double _delta)
		{
			...
			for(unsigned i=0; i<L; i++)
			{
				...
				last_excited[i].resize(L, tau); // last_excited is zero at the beginning
		...
		//	total_bonds = L*L;
			...
			gsl_rng_set(r,3);//random number generator seeded with time
			...
			gsl_rng_set(r,9);//random number generator seeded with time
		...
		void operate()//one time step
		{
			...
			for(int i=0;i<L;i++)
			{
				for(int j=0; j<L; j++)
				{
					...
					new_state[i][j] *= (state[i][j]<refractory[i][j]);//promote to resting after end of refactory 
		...
		void interact(sf::RenderWindow & renderWindow, int length) 
		{
			...
			case sf::Event::KeyPressed:
				...
				for(int i = 0; i<10; i++)
				{
					for(int j = 0; j<10; j++)
					{
						state[My/length+i][Mx/length+j] = 0;
						new_state[My/length+i][Mx/length+j] = 0;
						unablated[My/length+i][Mx/length+j] = 0;
					}
				}
		...
		int restitution(int i, int j)
	{
		...
		if (last_excited[i][j] < T) //acting on cells that were previously excited fast
		{
			...
			//return floor((1 - 0.5*exp(-4.0*last_excited[i][j] / (1.0*T)))*tau);
			return tau;
		}
		else
		{
