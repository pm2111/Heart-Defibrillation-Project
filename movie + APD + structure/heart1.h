...
public:
	...
	heart(int _L, int _T, int _tau, double _nu, double _epsilom, double _delta)
	{
		...
		for (unsigned i = 0; i<L; i++)
		{
			...
			last_excited[i].resize(L, 0); // last_excited is zero at the beginning
...
/*	double get_avg_refractory()
	...
	*/
	...
	void ablate(int x1, int y1, int x2, int y2, int x3, int y3, int x4, int y4)
