	...
	for (int nu1 = 19; nu1 < 21; nu1 += 1)
	{
		...
		for (int t = 0; t < time_steps; t++)
		{
			...
			if ( (t % 10000 < 1))
			{
				...
				double time = t;
				sstm.str("");
				sstm << time << "_" << nu  << "_" <<  light << "_" << restitution  << "_" << excited  << "_" << size << "_" << health << "_" << time_steps << ".txt";
		...
		sstm.str("");
		sstm  << nu  << "_" << structure << "_" <<  light << "_" << restitution  << "_" << excited  << "_" << size << "_" << health << "_" << time_steps << ".txt";
		outfile.open(sstm.str());
		for (int i = 0; i < size; i++)
		{
			for (int j = 0; j < size; j++)
