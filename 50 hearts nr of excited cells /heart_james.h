...
public:
	...
	heart(int _L, int _T, int _tau, double _nu, double _epsilom, double _delta)
	{
		...
		gsl_rng_set(r, time(NULL));//random number generator seeded with time for DYNAMICS
		//gsl_rng_set(r, 11);//random number generator seeded with time
	}
	...
	int restitution(int i, int j)
	{
		...
		{
			return floor((1 - 0.333*exp(-4.0*last_excited[i][j] / (1.0*T)))*tau);
			//return tau;
		}
		else
		{
